...
import { LIB_ITEMS, COLLECTIONS, SESSIONS, ALL_TAGS } from '../data'
...
export function Library() {
  ...
  const [srcFilter, setSrcFilter] = useState<'all' | 'ref' | 'sim'>('all')
  const [activeTags, setActiveTags] = useState(new Set(['night']))
    ...
    <div className="lib">
      <style>{`
        ...
        .lib-side .sh {
          ...
          padding: 10px 8px 6px; display: flex; justify-content: space-between; align-items: center;
        }
        ...
        .lib-side .it {
          ...
          border-radius: var(--r-xs); cursor: pointer; font-size: 12px; color: var(--fg);
        }
        ...
        .lib-sub {
          ...
          background: var(--bg-subtle); font-size: 11px; color: var(--fg-muted);
          overflow-x: auto;
        }
        ...
        .lib-card .thumb .src-badge { position: absolute; top: 6px; left: 6px; z-index: 2; }
        .lib-card .thumb .fav-btn {
        ...
        .lib-card .cname { font-family: var(--font-mono); font-size: 11px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        ...
        .lib-table { background: var(--panel); border: 1px solid var(--border); border-radius: var(--r-md); overflow: hidden; }
        ...
        .lib-table .th {
          ...
          color: var(--fg-muted); background: var(--bg-subtle); border-bottom: 1px solid var(--border);
        }
        .lib-table .tr { border-bottom: 1px solid var(--divider); font-size: 12px; cursor: pointer; }
        ...
        .lib-table .tr .tname { font-family: var(--font-mono); font-size: 12px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .lib-table .tr .tmono { color: var(--fg-muted); font-size: 11px; }
        .lib-table .tr .tthumb { width: 48px; height: 32px; border-radius: 3px; overflow: hidden; }
      ...
      <div className="lib-side">
        ...
        {COLLECTIONS.map(c => (
          <div key={c.id} className={`it ${filter === c.id ? 'active' : ''}`} onClick={() => setFilter(c.id)}>
            <Icon name={c.icon as Parameters<typeof Icon>[0]['name']} size={13} className="ic" />
            <span>{c.name}</span>
      ...
      <div className="lib-main">
        <div className="lib-bar">
          ...
          <Seg<'all' | 'ref' | 'sim'>
            ...
            options={[{ v: 'all', label: 'All' }, { v: 'ref', label: 'Ref' }, { v: 'sim', label: 'Sim' }]}
          />
          <button className="btn sm"><Icon name="filter" size={12} /> Filter</button>
          <Seg<'grid' | 'list'>
            value={view} onChange={setView}
            options={[{ v: 'grid', label: '', icon: 'grid' }, { v: 'list', label: '', icon: 'rows' }]}
          />
        ...
        <div className="lib-content">
          {view === 'grid' ? (
            <div className="lib-grid">
              {filtered.map(it => (
                <div key={it.id} className={`lib-card ${selected.has(it.id) ? 'sel' : ''}`} onClick={() => toggleSel(it.id)}>
                  <div className="thumb">
            ...
            <div className="lib-table">
              <div className="th">
                <span /><span /><span>Name</span><span>Source</span>
                <span>Tags</span><span>Session</span><span>Dimensions</span><span>Date</span><span />
              </div>
              {filtered.map(it => (
                <div key={it.id} className={`tr ${selected.has(it.id) ? 'sel' : ''}`} onClick={() => toggleSel(it.id)}>
                  ...
                  <span className="tmono">{it.session}</span>
                  <span className="tmono">{it.w}×{it.h}</span>
                  <span className="tmono">{it.date}</span>
