...
type IconName =
  | 'home' | 'sparkle' | 'folder' | 'search' | 'plus' | 'upload' | 'settings'
  | 'node' | 'list' | 'timeline' | 'grid' | 'rows' | 'split' | 'slider'
  | 'star' | 'tag' | 'play' | 'chev' | 'chevDown' | 'close' | 'more' | 'dots'
  | 'eye' | 'eyeOff' | 'reset' | 'wand' | 'camera' | 'img' | 'pipeline'
  | 'curve' | 'wheel' | 'histogram' | 'filter' | 'check' | 'link' | 'clock'
  | 'zap' | 'sun' | 'droplet' | 'layers'
...
const PATHS: Record<IconName, React.ReactNode> = {
  ...
  curve:     <><path d="M3 21 21 3"/><path d="M3 21c4 0 6-12 18-12"/></>,
...
export function Icon({ name, size = 14, stroke = 1.6, className = '', style = {} }: IconProps) {
  return (
    <svg
