...
function SliderCompare() {
  ...
  return (
    <div
      ...
      style={{ '--sx': `${slider}%` } as React.CSSProperties}
    ...
    >
      <div className="cv-slider-a">
        <span className="cv-label ref">A · Reference</span>
        <PhImg label="reference" hue={210} style={{ position: 'absolute', inset: 0, borderRadius: 0 }} />
      </div>
      <div className="cv-slider-b">
        <span className="cv-label sim" style={{ right: 10, left: 'auto' }}>B · Simulation</span>
...
export function CompareViewer({ mode, onMode, labelA = 'Reference', labelB = 'Simulation' }: CompareViewerProps) {
  ...
  return (
    <div className="cv">
      <style>{`
        ...
        .cv-label.ref { background: rgba(30,30,30,.75); }
        .cv-label.sim { background: color-mix(in oklch, var(--accent) 80%, black); }
        .cv-pane .info {
          position: absolute; bottom: 10px; left: 10px; z-index: 3;
          color: rgba(255,255,255,.85); font-family: var(--font-mono); font-size: 10px;
        ...
        .cv-bottom {
          ...
          background: var(--panel); font-size: 11px; color: var(--fg-muted);
        }
      ...
      <div className="cv-stage">
        ...
        {mode === 'slider' && <SliderCompare />}
        {mode === 'overlay' && (
          <div className="cv-pane" style={{ height: '100%' }}>
            <span className="cv-label sim">A/B · BLEND 50%</span>
