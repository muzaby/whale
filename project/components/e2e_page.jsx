/* E2E page — ties pipeline, compare, and tuning panel together */

const E2E = () => {
  const [pipeView, setPipeView] = React.useState("node"); // node / list / timeline
  const [cmpMode, setCmpMode]   = React.useState("split"); // split / slider / overlay
  const [sel, setSel]           = React.useState("tone");
  const [bypass, setBypass]     = React.useState(false);
  const [pipeH, setPipeH]       = React.useState(260);

  return (
    <div className="e2e" data-screen-label="E2E">
      <style>{`
        ...
        .e2e-resizer {
          height: 6px; background: var(--panel); border-top: 1px solid var(--border); border-bottom: 1px solid var(--border);
          cursor: row-resize; position: relative;
        }
        .e2e-resizer::after { content: ""; position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); width: 28px; height: 2px; background: var(--border-strong); border-radius: 2px; }
        ...
        .e2e-pipe-head h4 { margin: 0; font-size: 11px; text-transform: uppercase; letter-spacing: 0.06em; font-weight: 600; color: var(--fg-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-width: 0; }
      ...
      <div className="e2e-main">
        ...
        <div
          ...
          onMouseDown={(e) => {
            ...
            const move = (ev) => setPipeH(Math.max(140, Math.min(500, startH + (startY - ev.clientY))));
        ...
        <div className="e2e-pipe">
          <div className="e2e-pipe-head">
            ...
            <span className="badge mono">~{PIPELINE.reduce((a,_,i)=>a+(1.2+i*0.31),0).toFixed(1)} ms</span>
            <div className="spacer" style={{ flex: 1 }} />
            <Seg value={pipeView} onChange={setPipeView} options={[
