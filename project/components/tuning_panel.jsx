/* Tuning panel — curves, HSL, color wheels, LUT, histogram */
...
const ToneCurve = () => {
  ...
  const onDragPoint = (i, e) => {
    ...
    const move = (ev) => {
      ...
      setPoints(p => p.map((pt, idx) => idx === i ? { x: i === 0 ? 0 : i === p.length - 1 ? 255 : nx, y: ny } : pt));
    };
    ...
    <div className="tone-curve">
      <style>{`
        ...
        .tone-curve .chans button {
          ...
          padding: 3px 0; cursor: pointer; color: var(--fg-muted);
      ...
      `}</style>
      <div className="chans">
        <button className={channel === "luma" ? "active" : ""} onClick={() => setChannel("luma")}>LUMA</button>
        <button className={`r ${channel === "r" ? "active" : ""}`} onClick={() => setChannel("r")}>R</button>
        <button className={`g ${channel === "g" ? "active" : ""}`} onClick={() => setChannel("g")}>G</button>
        <button className={`b ${channel === "b" ? "active" : ""}`} onClick={() => setChannel("b")}>B</button>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`}>
        <defs>
        ...
        <g opacity="0.3">
          {Array.from({ length: 64 }, (_, i) => {
            ...
            const v = Math.max(2, 70 * Math.exp(-Math.pow((i - 32) / 22, 2)) + Math.random() * 8);
            return <rect key={i} x={x} y={H - pad - v} width={(W - pad * 2) / 64 - 0.5} height={v} fill={channel === "r" ? "var(--red)" : channel === "g" ? "var(--green)" : channel === "b" ? "var(--blue)" : "var(--luma)"} />;
          })}
        </g>
        {/* diagonal reference */}
        <line x1={pad} y1={H - pad} x2={W - pad} y2={pad} stroke="var(--border)" strokeWidth="0.5" strokeDasharray="2 2" />
        {/* curve */}
        <path d={path} fill="none" stroke={channel === "r" ? "var(--red)" : channel === "g" ? "var(--green)" : channel === "b" ? "var(--blue)" : "var(--fg)"} strokeWidth="1.5" />
        {points.map((p, i) => (
...
const ColorWheel = ({ label, pos = { x: 0, y: 0 }, onChange = () => {}, value = 0 }) => {
  const R = 50;
  ...
  return (
    <div className="wheel">
      <style>{`
        ...
        .wheel .yscale input {
          height: 3px; background: linear-gradient(to right, oklch(0.2 0 0), oklch(0.5 0 0), oklch(0.98 0 0));
          border-radius: 2px;
        }
      `}</style>
  ...
  return (
    <div>
      <style>{`
        ...
        .hsl-row button {
          flex: 1; height: 22px; border: 1px solid var(--border);
...
/* ─── Histogram ─────────────────────────────── */
const Histogram = () => {
  const W = 220, H = 86, pad = 4;
  const bins = 96;
  const gen = (peak, spread, scale) =>
    Array.from({ length: bins }, (_, i) =>
      Math.max(0, scale * Math.exp(-Math.pow((i - peak) / spread, 2)) + (Math.random() - 0.5) * 3)
    );
...
const TuningPanel = ({ blockId, onToggleBypass, bypass }) => {
  const block = PIPELINE.find(b => b.id === blockId) || PIPELINE[6];
  const type =
    ["tone"].includes(block.id) ? "curve" :
    ["hsl"].includes(block.id) ? "hsl" :
    ["wheel"].includes(block.id) ? "wheel" :
    ["wb"].includes(block.id) ? "wb" :
    ["lut"].includes(block.id) ? "lut" :
    ["nr","sharp"].includes(block.id) ? "detail" :
    "generic";
  ...
  return (
    <div className="tune">
      ...
      <div className="body">
        {/* Histogram is always visible at top for context */}
          ...
          <div className="section">
            ...
            <div className="col" style={{ gap: 10 }}>
              {Object.entries(block.params).map(([k, v]) => (
                <Slider key={k} label={k.charAt(0).toUpperCase() + k.slice(1)} value={v} />
              ))}
            </div>
          </div>
        )}
...
Object.assign(window, { TuningPanel });
